# Capítulo 3 — Mecánica cuántica

<span class="qc-badge gold">Clase 5</span>

La física detrás del formalismo: de la mecánica clásica a la cuántica, la ecuación de Schrödinger y los cinco postulados.

## Leer el capítulo

[:material-book-open-page-variant: Leer el Capítulo 3 (PDF)](../assets/libro/capitulo-03.pdf){ .md-button .md-button--primary }

## Diapositivas

[:material-file-download: Descargar diapositivas del Capítulo 3](../assets/slides/cap03_mecanica_cuantica.pdf){ .md-button }

## Temas

- De lo clásico a lo cuántico: Lagrange, Hamilton y el teorema de Noether.
- El puente de Poisson a los conmutadores: \( \{q,p\}=1 \to [\hat q,\hat p]=i\hbar \).
- La ecuación de Schrödinger, dependiente e independiente del tiempo.
- Cuantización canónica y el qubit superconductor.
- El oscilador armónico y los operadores escalera \( \hat a,\hat a^\dagger \).
- Los cinco postulados de la mecánica cuántica y el teorema de Bell.

## Enlaces del capítulo

- Ejercicios: [próximamente]
