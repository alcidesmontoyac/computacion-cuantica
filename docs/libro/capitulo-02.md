# Capítulo 2 — Fundamentos matemáticos

<span class="qc-badge gold">Clase 4</span>

El lenguaje del curso: álgebra lineal compleja y notación de Dirac, con las matrices de Pauli y el espacio de Hilbert como destino.

## Diapositivas

[:material-file-download: Descargar diapositivas del Capítulo 2](../assets/slides/cap02_fundamentos.pdf){ .md-button .md-button--primary }

## Temas

- Mezcla clásica vs. superposición cuántica: probabilidades vs. amplitudes.
- Regla de Born y el término de interferencia.
- Notación de Dirac: bra, ket, producto interno; sesquilinealidad y completitud.
- Bases estándar y de Hadamard; producto externo y proyectores.
- Autovalores y autovectores; el ejemplo resuelto de las matrices de Pauli.
- Hermíticas (observables), unitarias (evolución) y el teorema espectral.
- Producto tensorial: el origen del crecimiento \(2^n\); el espacio de Hilbert.

## Enlaces del capítulo

- Clase en video: [Clase 4](../curso/clase-04.md)
- Práctica: [Bra-ket y matrices de Pauli](../practicas/practica-02-braket-pauli.md)
- Ejercicios: [Capítulo 2](../ejercicios/capitulo-02.md)
